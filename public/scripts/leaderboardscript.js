...
async function displayLeaderboard(season, sugu, vanusegrupp, name) {
    const queryParams = new URLSearchParams({
        season: season || '', // Send empty string for all seasons
        sugu: sugu || '',
    ...
    if (data.length === 0) {
        ...
        document.querySelectorAll('.leaderboard-table tbody tr').forEach(row => {
            row.addEventListener('click', async function() {
                ...
                console.log('Row dataset:', this.dataset);
...
async function fetchAndDisplayPointsDetails(athleteId, vanusegrupp, hooaeg, eesnimi, perenimi) {
    console.log('Fetching details for athlete:', athleteId, 'vanusegrupp:', vanusegrupp, 'hooaeg:', hooaeg);
...
function displayPointsDetails(pointsData, athleteName) {
    ...
    athleteNameElement.textContent = `Sportlane: ${athleteName}`; // Display the athlete's name
...
document.getElementById('leaderboardFilterForm').addEventListener('submit', async function(event) {
    ...
    const season = document.getElementById('seasonInput').value || ''; // Empty string for all seasons
