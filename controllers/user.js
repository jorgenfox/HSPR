...
exports.userSignup = async (req, res) => {
    ...
    if (password !== confirm_password) {
        return res.json({ success: false, error: 'Salasõnad ei ühti. Palun proovige uuesti.' });
    }
    ...
    try {
        ...
        if (existingUser.length > 0) {
            return res.json({ success: false, error: 'See e-mail on juba registreeritud. Palun kasuta teist e-posti aadressi.' });
        }
        ...
        res.json({ success: true, message: 'Konto edukalt loodud! Palun logi sisse.', redirectUrl: '/hspr/user/login' });
    } catch (err) {
        console.error('Error during user registration:', err);
        res.status(500).json({ success: false, error: 'Registreerumisel tekkis viga. Palun proovige hiljem uuesti.' });
    }
};
